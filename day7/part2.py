...
def concatenate(equation, spots):
    concatenated = []
    reduced_spots = []
    status = False
    for i, operator in enumerate(spots):
        if status == False:
            concatenated.append(equation[i])
        if operator == '|':
            concatenated[-1] = int(str(concatenated[-1]) +
                                   str(equation[i + 1]))
            status = True
        else:
            reduced_spots.append(operator)
            status = False

    if not status:
        concatenated.append(equation[-1])

    return concatenated, reduced_spots
